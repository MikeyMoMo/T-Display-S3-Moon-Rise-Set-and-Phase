...
/*******************************************************************************************/
{
  ...
  //  and have the scroll onto screen more naturally vs. having them pop up fully formed.
  ...
  count = 35;  // Serial.printf("Creating %i stars.\r\n", count);
...
/*******************************************************************************************/
{
  ...
  /* Well, the following needs a little explanation.  It took a while to figure out the order
      ...
      starfield gets truncated and that's exactly what I wanted to happen. So, now, we have a
      black background and a starfield in the left side of the spriteBG sprite.
      ...
      to get rid of the black corners.  To do this, I have to push the moon sprite onto another
      sprite with the invisible color (black) noted so those pixels will not be copied to the
      second sprite.  That is done with the pushToSprite, 4 code lines below.  The target
      sprite already has a black background and you would not see the four corners but there is
      still a problem. Those four corners would obscure the stars that happened along where
      they are.  So, they have to be lopped off with the pushToSprite setting black as the
      invisible color to leave them behind.  So, now, we have a black sprite that has had a
      slightly wide starfield pushed onto it and then a moon image that is now round, without
      any corners pressed on top of the start field.  Yes, that obscures some stars but that's
